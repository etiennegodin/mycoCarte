...
occurences_datasets_to_merge = { 'foretOuverte': foretOuverte_df,
            'bioclim' :bioclim_df,
            'bias' : bias_df,
            'grid': grid_df
}

merged_occurences_df = mergeDataset(occurences_df, occurences_datasets_to_merge)
merged_agg_occurences_df = mergeDataset(aggregated_occurences_df, occurences_datasets_to_merge)


merged_occurences_df.to_csv('data/interim/model/preprocessedOccurencesData.csv')
merged_agg_occurences_df.to_csv('data/interim/model/preprocessedAggregatedOccurencesData.csv')
...
# Create grid data to test 
grid_datasets_to_merge = { 'foretOuverte': foretOuverte_df,
            'bioclim' :bioclim_df,
            'bias' : bias_df
}
merge_test_df = mergeDataset(grid_df, grid_datasets_to_merge)
merge_test_df.to_csv('data/interim/model/testData.csv')

print('-'*100)
print(merge_test_df.shape)
print(merge_test_df.head())
